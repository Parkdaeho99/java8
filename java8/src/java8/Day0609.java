...
public class Day0609 {
	...
	public static void main(String[] args) {
		...
		while (true) {
			 ...
			 */
			/*
			 * String은 기본 자료형이 아니고 객체라서 값 자체를 가지지 않고, 값을 가진 공간을 참조한다. 그러므로 == 연산자로 비교하면 안되고,
			 * equals를 써서 비교해야 한다. == 연산자는 양 옆의 값을 비교하는 건데, 아래 코드에서 m_hand == "a" 부분은
			 * m_hand가 String이기 때문에 값으로 주소값을 가지고 있기 때문에 == 연산자를 사용하면 m_hand의 주소값과 "a"가 같냐고
			 * 묻는 것?
			 * 
			 */
			// System.out.println(System.identityHashCode("a")); // 421
			...
			System.out.println("현재점수:" + score + "       " + "현재라이프" + life);
			if (life == 0)
				break;
		}
		sc.close();
